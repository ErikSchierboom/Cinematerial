namespace Cinematerial
{
    ...
    // The code below was created by Jeroen-bart Engelen and can be found at: http://www.codeproject.com/Articles/9183/A-C-implementation-of-Unix-crypt
    ...
    /// </remarks>
    internal sealed class UnixCrypt
    {
        /// <value>
        ...
        private static void PermOperation(uint firstInt, uint secondInt, uint thirdInt, uint fourthInt, uint[] operationResults)
        {
            ...
            // for more details.
            var tempInt = ((firstInt >> (int) thirdInt) ^ secondInt) & fourthInt;
        ...
        private static uint HPermOperation(uint firstInt, int secondInt, uint thirdInt)
        {
            ...
            // unnecessary casting.
            var tempInt = ((firstInt << (16 - secondInt)) ^ firstInt) & thirdInt;
            var returnInt = firstInt ^ tempInt ^ (tempInt >> (16 - secondInt));
        ...
        private static uint[] SetDESKey(byte[] encryptionKey)
        {
            ...
            var firstInt = FourBytesToInt(encryptionKey, 0);
            var secondInt = FourBytesToInt(encryptionKey, 4);

            var operationResults = new uint[2];
            ...
            for (var index = 0; index < m_desIterations; index++)
            {
        ...
        private static uint[] Body(uint[] schedule, uint firstSaltTranslator, uint secondSaltTranslator)
        {
            ...
            var singleOutputKey = new uint[2];
        ...
        public static string Crypt(string textToEncrypt)
        {
            ...
            var maxGeneratedNumber = m_encryptionSaltCharacters.Length;
            int randomIndex;
            ...
            var encryptionSalt = encryptionSaltBuilder.ToString();
            var encryptedString = Crypt(encryptionSalt, textToEncrypt);
        ...
        {
            var isSaltTooSmall = (encryptionSalt.Length < 2);
            if (isSaltTooSmall)
            {
                throw new ArgumentException("The encryptionSalt must be 2 characters big.");
            }

            var firstSaltCharacter = encryptionSalt[0];
            var secondSaltCharacter = encryptionSalt[1];
            ...
            var encryptionBuffer = new StringBuilder("*************");
            ...
            var firstSaltTranslator = m_saltTranslation[Convert.ToUInt32(firstSaltCharacter)];
            var secondSaltTranslator = m_saltTranslation[Convert.ToUInt32(secondSaltCharacter)] << 4;
            ...
            for (var index = 0; index < encryptionKey.Length && index < textToEncrypt.Length; index++)
            {
                var shiftedCharacter = Convert.ToInt32(textToEncrypt[index]);
                encryptionKey[index] = (byte) (shiftedCharacter << 1);
            }

            var schedule = SetDESKey(encryptionKey);
            var singleOutputKey = Body(schedule, firstSaltTranslator, secondSaltTranslator);
            ...
            {
                passwordCharacter = 0;
                for (var secondIndex = 0; secondIndex < 6; secondIndex++)
                {
                    passwordCharacter <<= 1;
